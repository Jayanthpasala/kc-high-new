...
const safeParseAIResponse = (text: string | undefined) => {
  ...
  try {
    ...
    // Stage 2: Discover first array or object if AI included conversational text
...
export const ProductionPlanning: React.FC = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert("AI Sync Failed: Document data could not be parsed. Ensure the file is a clear image. In deployment, verify your API key.");
    } finally { setIsProcessing(false); }
  };

  const handleApproveAll = async () => {
    if (missingRecipesCount > 0) {
      alert("Validation Alert: Link all dishes to a Master Specification before syncing.");
      return;
    }
    setIsProcessing(true);
    try {
      ...
      {isProcessing && (
        <div className="fixed inset-0 z-[110] bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-6 text-center animate-in fade-in duration-300">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl max-w-sm w-full">
            ...
            <h3 className="text-2xl font-black text-slate-900">AI Extracting...</h3>
            <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mt-4">Analyzing Menu & Populations</p>
          </div>
        </div>
      )}
      ...
      {view === 'UPLOAD' && (
        <div className="max-w-4xl mx-auto space-y-12 py-10 animate-in slide-in-from-bottom-10 duration-500">
           <div className="text-center space-y-4">
              ...
              <h3 className="text-4xl font-black text-slate-900 tracking-tighter">Smart Import</h3>
              <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest max-w-sm mx-auto">Upload weekly documents to auto-link dishes and population counts for accurate production logs.</p>
           </div>
      ...
      {view === 'REVIEW' && (
        <div className="space-y-10 animate-in fade-in duration-500">
           <div className="flex justify-between items-center">
              <h3 className="text-2xl font-black text-slate-900 uppercase">Review Extractions</h3>
              <div className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border-2 ${missingRecipesCount > 0 ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                 {missingRecipesCount > 0 ? `${missingRecipesCount} Unlinked Dishes` : 'System Verified'}
              </div>
           </div>
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {pendingPlans.map((plan, pi) => {
                // Fix: Explicitly type reduce parameters to avoid unknown operator issues from Object.values
                const total = Object.values(plan.headcounts || {}).reduce((a: number, b: any) => a + (Number(b) || 0), 0);
                return (
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
                 {dayPlan ? (
                    <div className="space-y-8">
                       ...
                       <button onClick={handleSaveDay} className="w-full bg-slate-900 text-white py-6 rounded-3xl font-black uppercase text-xs tracking-widest hover:bg-emerald-600 transition-all">Save Changes</button>
                    </div>
                 ) : (
